...
def sn_Conv2d(din, dout, k, s, p):
    conv2d = nn.Conv2d(din, dout, k, s, p)
    sn_conv2d = torch.nn.utils.parametrizations.spectral_norm(conv2d, name="weight", n_power_iterations=1)
...
class LinearConditionalMaskLogits(nn.Module):
    ...
    def __init__(self, nc, nlabels, sn=False):
        super().__init__()
        fc = nn.Linear(nc, nlabels)

        if sn:
            self.fc = torch.nn.utils.parametrizations.spectral_norm(fc, name="weight", n_power_iterations=1)
        else:
            self.fc = fc
