...
class Generator(nn.Module):
    def __init__(self,
        ...
        self.feat_size = kwargs["size"] // 8
        if conditioning == 'embedding':
            self.get_latent = blocks.LatentEmbeddingConcat(nlabels, embed_dim)
            self.fc = nn.Linear(z_dim + embed_dim, self.feat_size * self.feat_size * ngf * 8)
        elif conditioning == 'unconditional':
            self.get_latent = blocks.Identity()
            self.fc = nn.Linear(z_dim, self.feat_size * self.feat_size * ngf * 8)
        else:
    ...
    def forward(self, input, y):
        ...
        out = self.fc(out)
        out = out.view(out.size(0), -1, self.feat_size, self.feat_size)
...
class Discriminator(nn.Module):
    def __init__(self,
        ...
        self.conv1 = nn.Sequential(blocks.sn_Conv2d(nc * pack_size, ndf, 3, 1, 1), nn.LeakyReLU(0.1))
        self.conv2 = nn.Sequential(blocks.sn_Conv2d(ndf, ndf, 4, 2, 1), nn.LeakyReLU(0.1))
        self.conv3 = nn.Sequential(blocks.sn_Conv2d(ndf, ndf * 2, 3, 1, 1), nn.LeakyReLU(0.1))
        self.conv4 = nn.Sequential(blocks.sn_Conv2d(ndf * 2, ndf * 2, 4, 2, 1), nn.LeakyReLU(0.1))
        self.conv5 = nn.Sequential(blocks.sn_Conv2d(ndf * 2, ndf * 4, 3, 1, 1), nn.LeakyReLU(0.1))
        self.conv6 = nn.Sequential(blocks.sn_Conv2d(ndf * 4, ndf * 4, 4, 2, 1), nn.LeakyReLU(0.1))
        self.conv7 = nn.Sequential(blocks.sn_Conv2d(ndf * 4, ndf * 8, 3, 1, 1), nn.LeakyReLU(0.1))

        self.feat_size = kwargs["size"] // 8
        if conditioning == 'mask':
            self.fc_out = blocks.LinearConditionalMaskLogits(
                ndf * 8 * self.feat_size * self.feat_size, nlabels, sn=True)
        elif conditioning == 'unconditional':
            self.fc_out = blocks.LinearUnconditionalLogits(
                ndf * 8 * self.feat_size * self.feat_size, sn=True)
        else:
            raise NotImplementedError(
