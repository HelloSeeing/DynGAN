...
class Evaluator(object):
    ...
    def compute_fid(self):
        fake_imgs, real_imgs = self.get_fake_real_samples(N=self.fid_nsamples)
        fake_imgs = np.stack([img.numpy() for img in fake_imgs], axis=0)
        real_imgs = np.stack([img.numpy() for img in real_imgs], axis=0)
        fid_value = fid.calculate_frechet_inception_distance(fake_imgs=fake_imgs, real_imgs=real_imgs, dims=2048, device="cuda:0")
