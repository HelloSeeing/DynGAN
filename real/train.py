...
def main():
    ...
    with open(os.path.join(out_dir, "config.yaml"), 'w') as f:
        yaml.dump(config, f, Dumper=yaml.SafeDumper, encoding='utf-8')
    ...
    checkpoint_io.register_modules(clusterer=clusterer)

    # Load checkpoint if it exists
    it = utils.get_most_recent(checkpoint_dir, 'model') if args.model_it == -1 else args.model_it
    # it, epoch_idx, loaded_clusterer = checkpoint_io.load_models(it=it, load_samples=True)
    it, epoch_idx = checkpoint_io.load_models(it=it, load_samples=True)
    # it = -1

    success = clusterer.load(os.path.join(checkpoint_dir, f"clusterer{it:08d}.pth"))
    ...
    if not success:
        ...
        clusterer.save(os.path.join(checkpoint_dir, f"clusterer{it:08d}.pth"))
        np.savez(os.path.join(checkpoint_dir, 'cluster_samples.npz'), x=x_cluster)
        if config['clusterer']['name'] == "dyngan":
            for c in range(clusterer.kt):
                idx = np.where(clusterer.x_labels == c)[0]
                x = clusterer.x[idx[:64]]
                logger.add_imgs(x, 'cluster-%04d' % c, 0)
    else:
    ...
    evaluator = Evaluator(
        ...
        fid_nsamples=config['training']['fid_nsamples'], )
    ...
    while it < args.nepochs * len(train_loader):
        ...
        for x_real, y in train_loader:
            ...
            # (iv)
            if config['clusterer']['name'] == "dyngan":
                if it % config['clusterer']['detect_every'] == 0 and it > config['training']['burnin_time']:
                    # print mode collapse detection result
                    status, nums = clusterer.update_mode_collapse_status(discriminator=discriminator, N=10,)
                    print(f'[epoch {epoch_idx}, it {it}] mode collapse status: {status :.2f}')
                    print(f'[epoch {epoch_idx}, it {it}] status distribution: {nums}')
            ...
            if it % config['training']['recluster_every'] == 0 and it > config['training']['burnin_time']:
                ...
                if config['clusterer']['name'] == "dyngan":
                    if status > config['clusterer']['kwargs']['tau']:
                        clusterer.recluster(discriminator=discriminator, x_batch=x_real)
                        for c in range(clusterer.kt):
                            idx = np.where(clusterer.x_labels == c)[0]
                            x = clusterer.x[idx[:64]]
                            logger.add_imgs(x, 'cluster-%04d' % c, it)
                else:
                    clusterer.recluster(discriminator=discriminator, x_batch=x_real)
            ...
            # visualize labels
            if config['clusterer']['name'] == "dyngan":
                if it % config['training']['label_every'] == 0 and it > config['training']['burnin_time']:
                    cluster_labels = np.zeros(shape=(clusterer.kt, nlabels))
                    for x_, y_ in train_loader:
                        x_ = x_.to(device)
                        c_ = clusterer.get_labels(x_, y_).detach().cpu().numpy()
                        
                        for cl, yl in zip(c_, y_):
                            cluster_labels[cl, yl] += 1

                    logger.add_label_hist(labels=cluster_labels, class_name="full_labels", it=it)

                    cluster_labels = np.zeros(shape=(clusterer.kt, len(train_dataset.classes)))
                    cluster_true_labels = np.zeros(shape=(clusterer.kt, len(train_dataset.classes)))
                    st = 0
                    en = st + batch_size
                    while en < x_cluster.size(0):
                        x_ = x_cluster[st : en].to(device)
                        y_ = y_cluster[st : en]
                        c_ = clusterer.get_labels(x_, y_).detach().cpu().numpy()
                        c_t = clusterer.x_labels[st : en]

                        for cl, ct, yl in zip(c_, c_t, y_):
                            cluster_labels[cl, yl] += 1
                            cluster_true_labels[ct, yl] += 1

                        st = en
                        en = min(st + batch_size, x_cluster.size(0))

                    logger.add_label_hist(labels=cluster_labels, class_name="cluster_label", it=it)
                    logger.add_label_hist(labels=cluster_true_labels, class_name="cluster_true_label", it=it)

            # (i) Sample if necessary
            if it % config['training']['sample_every'] == 0:
                if config['clusterer']['name'] == "dyngan":
                    sample_nlabels = clusterer.kt
                print('Creating samples...')
                y_test = clusterer.get_labels(x_test, y_test)
                x = evaluator.create_samples(z_test, y_test.to(device))
